...
問題バンクは171問、全て`status:"active"`(stockは0)。**初回リリース5カテゴリ(食事・お金・恋愛・季節行事・健康カラダ)は各20問に到達**(2026-08-16、iOS化準備の一環でWebSearchによる実データ収集44問を追加+既存stock5問(自炊/外食・元恋人連絡先・七夕短冊・GW旅行・ダイエット経験)をactiveに昇格して達成)。20問到達により、この5カテゴリはPart1(固定10問)に加えてPart2(残り10問からランダム)が初めて出現するようになった。残り6カテゴリ(住まい身支度12/余暇娯楽12/仕事働き方12/デジタル習慣12/持ち物12/ジンクス運試し11)は`launched:false`のため未公開のまま、20問への拡充は未着手(下記残タスク参照)。全問実データ根拠・2択にタグ付け済み。仮置き(実データなし)の問題は引き続き全廃したまま。`server/data/questions-seed.json`への追記で継続する運用(追記だけで既存の実票・履歴を壊さず反映されることを確認済み。2026-08-16の追加でも実データ(is_dummy=0)の投票・セッションは0件のまま不変であることを確認済み)。
...
- ~~各カテゴリの問題数を20問(active)まで拡充する(2026-08-16指示、絶対対応)~~ → 初回リリース5カテゴリ分は2026-08-16に完了(上記「現状」参照)。**残り6カテゴリ(住まい身支度・余暇娯楽・仕事働き方・デジタル習慣・持ち物・ジンクス運試し)は`launched:false`で未公開のため今回は対象外にした。公開範囲を広げるタイミングで同様に20問へ拡充すること**
...
### devcontainer内(アカウント不要)で完結する範囲の対応(2026-08-16)

ユーザー本人のアカウント作成・支払い・macOS環境が必須な項目(VPS契約・Apple Developer Program登録・App Store Connect提出・Xcodeビルド)を除き、この devcontainer(Linux)内で完結できる範囲を一括対応した。広告(AdMob)実装は引き続き対象外(ユーザー指示)。

- **各カテゴリ20問拡充**: 上記「現状」参照。5カテゴリ×49問(WebSearch実データ収集44問+既存stock5問の昇格)で完了
- **サーバー堅牢性**(`server/index.js`): `express.json({ limit: '1mb' })`でペイロード上限、`Content-Security-Policy`ヘッダーを追加(詳細は`SPEC.md`「セキュリティヘッダー」)。`npm audit`は0件
- **プライバシーポリシー**: `www/privacy.html`を作成、Homeフッターからリンク。App Store Connect提出時のURL項目にはこのページの本番URLを充てる想定(VPSデプロイ後に確定)
- **Capacitor設定ファイル**: `capacitor.config.json`を作成(`server.url`は本番ドメイン未確定のためプレースホルダ)、`@capacitor/core`・`@capacitor/cli`を導入。`npx cap add ios`はCocoaPods/Xcodeが必要でLinux上では完結しないため未実行
- **App Store用マスターアイコン**: `www/icons/icon-1024.png`(1024x1024)を用意。@2x/@3x等の展開は`ios/`プロジェクト作成後に`@capacitor/assets`で行う
