...
| テーブル | 役割 | 備考 |
|---|---|---|
| `categories` | お題のカテゴリ | `id`/`label`/`sort_order` |
...
| `quiz_sessions` | 10問1セット完了時に確定するセッション記録 | `match_count`/`total_count`/`session_tier`をセッション確定時点のスナップショットとして保存 |
| `votes` | 1問への1回答 | `is_dummy`(1=シード投入のダミー票)、`voter_id`(端末識別、下記)、`majority_option_id_at_vote`(投票時点の多数派スナップショット、下記) |
...
マイグレーション(`server/db.js`の`migrate()`): `CREATE TABLE IF NOT EXISTS`では既存テーブルへの列追加ができないため、`votes.voter_id`と`topics.category`は起動時に個別に`ALTER TABLE`で追いかけている。本番データが増えたら正式なマイグレーション手順に切り替える想定。

## シード(`server/seed.js`)

起動のたびに`server/data/{attributes,categories,questions}-seed.json`を読み込んで反映する。

- `categories`・`attributes`・`attribute_values`: UPSERT/`INSERT OR IGNORE`で同期
- `topics`: `question`/`category`/`status`は毎回UPSERTで最新化(JSON追記だけで既存の実票・履歴を壊さず反映できる)
- `options`: `INSERT OR IGNORE`(一度作った選択肢の`label`は変えない)
- ダミー票(`dummyVotes`): そのお題に一件もダミー票がなければ`questions-seed.json`の`dummyVotes`比率通りに`is_dummy=1`の票を投入。既に入っていれば再投入しない(再起動のたびに水増しされるのを防ぐ)

## 多数派判定ロジック(`server/majority.js`)

- 母集団は「ダミー票 + 実票」を合算するのが基本(自己増殖的に育つ設計)
- **実データ閾値(`REAL_VOTE_MAJORITY_THRESHOLD = 100`)**: あるお題について実データ(`is_dummy=0`、同一`voter_id`は最新1票のみ、`server/votes-dedup.js`)が100件を超えたら、以後そのお題は実データのみを多数派判定の母集団とし、不要になったダミー票(`is_dummy=1`)を`DELETE`する。一度100件を超えたら二度とダミー票には戻らない(削除済みのため)
...
- `voter_id`がNULLの行(ダミー票・旧データ)はそれぞれ個別の1票としてそのまま数える
...
閾値は2026-08-15、`questions-seed.json`のダミー票比率を使ったモンテカルロシミュレーション(2万セッション試算)で検証済み。元は個性派境界0.2だったが「唯一無二」到達率が0.02%(実質到達不能)だったため0.3に引き上げ、約0.4%まで緩和した。
...
| `GET /api/categories` | active問題が1問以上あるカテゴリのみ、`sort_order`順 |
...
- 残り6カテゴリ(住まい身支度/余暇娯楽/仕事働き方/デジタル習慣/ジンクス運試し/持ち物)は`status:stock`のまま。10問に届くかカテゴリ統合するか未確定
- 属性「靴下(右足/左足)」は仮置きのまま(信頼できる出典が見つかっていない)
