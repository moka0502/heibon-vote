...
- **完全互角(tie)の扱い**(2026-08-18、実プレイFB「完全互角の時だけ。どんなにちっちゃい差でも、多数決じゃなければ×」): 投票前の分布で**上位2択の得票が完全に同数(ぴったり五分五分)**のときだけ、**どちらを選んでも一致扱い**にする(わずかでも差があれば少数派は従来通り不一致)。実装上は`majority_option_id_at_vote`に「選んだ選択肢」を保存する(→`isMajorityMatch=true`、セッションの`match_count`でも一致として数えられ、満点判定も崩れない)。レスポンスに`isTie`を返し、クライアントは「ぴったり五分五分! どちらを選んでも正解です」と表示する
- クライアントには、その一票を含めた最新の内訳(`percentages`)と、その一票が多数派と一致したか(`isMajorityMatch`)、完全互角か(`isTie`)を返す。`percentages`は`server/majority.js`の`percentagesFor()`で計算し、最後の選択肢だけ「残り」として算出することで合計が必ず100%になるようにしている(各選択肢を独立に`Math.round`すると101%等になる実例が確認されたため、2026-08-16に修正)
- レスポンスには生の得票数`voteCounts`(`{optionId: count}`)も含める。**接戦の小数表示**(2026-08-18、実プレイFB「実態は50.1対49.9なのに『50/50』と表示されると、互角に見えるのに少数派が✕になる理由が分からず怖い」): 2択で両方が同じ整数%に丸まる(=49.5〜50.5%帯)ときだけ、クライアント(`renderQuestionFeedback`)は`voteCounts/totalVotes`から小数第1位まで(例「49.5% / 50.5%」)を表示する。これにより「50/50に見えるのに✕」の違和感を解消しつつ、完全同数(`isTie`)でない限り当たり判定自体は多数決どおりに保つ
