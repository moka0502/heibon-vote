...
function showSuggestionForm() {
  mount(
    ...
    renderSuggestionForm(async (text, kind) => {
      await api.postSuggestion(text, kind);
      mount(renderSuggestionThanks(text, showHome, kind), showHome);
    }, showHome),
    showHome
  );
}
...
async function startQuiz(category, categoryLabel, part) {
  ...
  try {
    ...
    const fullCategoryLabel = categoryLabel && part ? `${categoryLabel} その${part}` : categoryLabel;
